  ...
  dashboardPage(
    ...
    dashboardSidebar(
      ...
      sidebarMenu(id="menu", style="font-size: 15px",
                  ...
                  menuItem("Créez un graphique", tabName = "Description", icon = icon("bar-chart")),
                  menuItem("Créez une carte", tabName = "Carte", icon = icon("map-o")),
                  menuItem("Créez une carte d'évolution", tabName = "Carte_evol", icon = icon("percent")),
                  hr(),
                  ...
                  selectInput(
                    ...
                    label = "Culture",
                    choices = list(
                      Oléagineux = c("Colza d'hiver" , "Soja" , "Tournesol"),
                      Protéagineux = c("Féverole" , "Lupin" , "Pois protéagineux" , "Soja")),
    ...
    dashboardBody(
      ...
      tabItems( #TabItems
        ...
        # SHEET 1 : HOME PAGE
        # ----------------------
        tabItem(tabName = "Accueil", class = "one",
                #Texte d'acceuil
                ...
                column(12,align="justify", style="font-family: 'times'; font-size:13pt",
                       fluidRow(
                         "Bienvenue dans l'outil de création de cartes SRP (Surfaces, Rendements et Productions) UE et Monde",
                         a("Terres Inovia",href="http://www.terresinovia.fr/",target="_blank()"), "et" ,a("Terres Univia",href="http://www.terresunivia.fr/",target="_blank()"),
        ...
        tabItem(tabName = "Donnees",
                ...
                fluidRow(
                  box(title="Données SRP UE et Monde", status="primary", solidHeader=TRUE, width=12, collapsible=TRUE,
                  ...
                  box(title="Paramétrage",status="primary",solidHeader=TRUE,width=12,collapsible=TRUE,
                      column(4,
                             sliderInput("annees.sheet.3", "Plage des années",
                                         min = data_ue %>% summarise(min(ANNEE)) %>% pull(),
                                         max = data_ue %>% summarise(max(ANNEE)) %>% pull(),
                                         value = c(data_ue %>% summarise(min(ANNEE)) %>% pull(), data_ue %>% summarise(max(ANNEE)) %>% pull()),
                                         sep=""
                             )
                      ),
                      column(6,
                             selectInput(inputId = "pays.sheet.3", label = "Sélectionner les pays", choices = data_ue %>% distinct(PAYS) %>% pull(), multiple = TRUE),
                             checkboxInput(inputId="aggregate.sheet.3", label = "Aggréger", value = FALSE)
                      )
                  ),
                               ...
                               tabBox(width = 12,
                                      ...
                                      tabPanel("Echelle",
                                               ...
                                               radioButtons("y.ajust.sheet.3","Ajuster l'axe Y",
                                                            ...
                                                            inline = TRUE),
                                               fluidRow(
                                                 column(4,numericInput("ymin.sheet.3","Min",value=data_ue %>% summarise(min(VALUE,na.rm=TRUE)) %>% pull())),
                                                 column(4,numericInput("ymax.sheet.3","Max",value=data_ue %>% summarise(max(VALUE,na.rm=TRUE)) %>% pull()))
                                               ),
                                               hr(),
                                               h3("Habillage texte"),
        ...
        tabItem(tabName = "Carte",
                ...
                fluidRow(
                  box(title="Paramétrage",status="primary",solidHeader=TRUE,width=12,collapsible=TRUE,
                      column(1,
                             numericInput("annee.sheet.4",label="Année",value=max(data_ue$ANNEE,na.rm=TRUE),min=min(data_ue$ANNEE,na.rm=TRUE),max=max(data_ue$ANNEE))),
                      
                      column(11,
                             fluidRow(
                               selectInput("choix.entite.sheet.4", "Sélectionnez les pays à représenter",
                    ...
                    column(6,
                           box(title="Carte & Données",status="primary",solidHeader=TRUE,width=12,collapsible=TRUE,
                               tabBox(width=12,
                                      ...
                                      tabPanel("Données",
                                               DT::dataTableOutput("df.sheet.4",width="90%"),
